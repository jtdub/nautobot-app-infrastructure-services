...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_INFRASTRUCTURE_SERVICES_xxx
namespace = Collection("infrastructure_services")
namespace.configure(
    {
        "infrastructure_services": {
            "nautobot_ver": "2.3.1",
            "project_name": "infrastructure-services",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.infrastructure_services.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.infrastructure_services.compose_http_timeout,
        "NAUTOBOT_VER": context.infrastructure_services.nautobot_ver,
        "PYTHON_VER": context.infrastructure_services.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.infrastructure_services.project_name}",
        f'--project-directory "{context.infrastructure_services.compose_dir}"',
    ]

    for compose_file in context.infrastructure_services.compose_files:
        compose_file_path = os.path.join(context.infrastructure_services.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.infrastructure_services.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.infrastructure_services.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.infrastructure_services.python_ver
    dockerfile_path = os.path.join(context.infrastructure_services.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.infrastructure_services.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.infrastructure_services.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.infrastructure_services.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations infrastructure_services"
...
@task(
    help={
        "version": "Version of Infrastructure Services to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} infrastructure_services"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("infrastructure_services") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " infrastructure_services.migrations"
        )
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'infrastructure_services/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.infrastructure_services.local):
